				  ...
				  struct vm_fault *vmf)
{
	...
	err = pmfs_get_xip_mem(mapping, vmf->pgoff, 1, &xip_mem, (long unsigned int *)&xip_pfn.val);
	if (unlikely(err)) {
	...
	pmfs_dbg_mmapv("[%s:%d] vm_start(0x%lx), vm_end(0x%lx), pgoff(0x%lx), "
			...
			(unsigned long)xip_pfn.val << PAGE_SHIFT);
...
void perfmodel_add_delay(int read, size_t size)
{
    ...
    if (do_bandwidth_delay) {
        ...
        //trace_printk("Bandwidth delay latency %u ns\n", extra_latency);

        // Lock for bandwidth delay.
        spin_lock(&pmfs_nvm_spinlock);
        emulate_latency_ns(extra_latency);
        spin_unlock(&pmfs_nvm_spinlock);
    } else {
    ...
    //trace_printk("nvm delay %lu ns - consumption %lu b\n", extra_latency, bandwidth_consumption);
#endif
