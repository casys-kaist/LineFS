...
int nrfsAddMetaToDirectory(nrfs fs, char* parent, char* name, bool isDirectory)
{
    ...
    //printf("nrfsAddMetaToDir: parent = %s name = %s\n", parent, name);
    int result = 0;
...
int nrfsRemoveMetaFromDirectory(nrfs fs, char* path, char* name)
{
	...
	//printf("nrfsRemoveMetaFromDir: parent = %s name = %s\n", path, name);
    int result = 0;
...
int nrfsWrite(nrfs fs, nrfsFile _file, const void* buffer, uint64_t size, uint64_t offset)
{
	...
	Debug::debugItem("path: %s size: %lx, offset: %lx", _file, size, offset);
	//printf("nrfsWrite: %s\n", _file);
#if defined DEBUG  &&  DEBUG == true
	//char *str = (char *)"write dump:";
	//hexDump(str, (void*)buffer, (int)size);
#endif
	...
	if(bufferExtentWriteReceive.result == true) {
		...
		for(int i = 0; i < (int)fpi.len; i++)
		{
			...
			//printf("nrfsWrite [fpi] i = %d node_id %d\n", i, fpi.tuple[i].node_id);
			...
			else
			{
				/*client->getRdmaSocketInstance()->RemoteWrite((uint64_t)((char*)buffer + length_copied), 
                                  ...
                                  fpi.tuple[i].size);*/
				client->RemoteWrite((uint64_t)((char*)buffer + length_copied), 
					              fpi.tuple[i].node_id,
                                  fpi.tuple[i].offset + DmfsDataOffset,
                                  fpi.tuple[i].size);
			}
			length_copied += fpi.tuple[i].size;
			//printf("write - length copied: %lu\n", length_copied);
		}
		gettimeofday(&end1, NULL);
		diff = 1000000 * (end1.tv_sec - start1.tv_sec) + end1.tv_usec - start1.tv_usec;
		WriteTime3 += diff;
		...
		//printf("Length copied: %d\n", (int)length_copied);
 		//ExtentWriteReceiveBuffer *test = (ExtentWriteReceiveBuffer *) &bufferGeneralReceive;
		//printf("write - receive addr: %p message type: %d result: %d offset: %lu key: %lu\n",
		//		test, test->message, test->result, test->offset, test->key);
		//if(bufferGeneralReceive.result == true)
			return (int)length_copied;
		//else
		//	return 1;
	} else {
		Debug::notifyError("write error for %s\n", _file);
		return -1;
	}
	return (int)length_copied;
}
...
int nrfsRead(nrfs fs, nrfsFile _file, void* buffer, uint64_t size, uint64_t offset)
{
	...
	Debug::debugCur("path: %s, size: %d, offset: %d", _file, size, offset);
	//printf("nrfsRead: %s\n", _file);
	gettimeofday(&start1, NULL);
	file_pos_info fpi;
	uint64_t length_copied = 0;
	...
	if(bufferExtentReadReceive.result == true) {
		...
		for(int i = 0; i < (int)fpi.len; i++)
		{
			Debug::debugItem("fpi: i = %d, node_id = %d,offset = %x, size = %d", 
				i, fpi.tuple[i].node_id, fpi.tuple[i].offset, fpi.tuple[i].size);
			//printf("nrfsRead [fpi] i = %d node_id %d\n", i, fpi.tuple[i].node_id);
			if(/*fpi.node_id[i] == (uint16_t)fs*/0)
			{
			...
			else
			{
				/*client->getRdmaSocketInstance()->RemoteRead((uint64_t)((char*)buffer + length_copied), 
                                  ...
                                  fpi.tuple[i].offset + DmfsDataOffset, 
                                  fpi.tuple[i].size);*/
				client->RemoteRead((uint64_t)((char*)buffer + length_copied), 
					              fpi.tuple[i].node_id,
                                  fpi.tuple[i].offset + DmfsDataOffset, 
                                  fpi.tuple[i].size);
			}
		...
		sendMessage(node_id, &bufferExtentReadEndSend, sizeof(ExtentReadEndSendBuffer), 
					&bufferGeneralReceive, sizeof(GeneralReceiveBuffer));
#if defined DEBUG  &&  DEBUG == true
		//char *str = (char *)"read dump:";
		//hexDump(str, buffer, (int)length_copied);
#endif
		gettimeofday(&end1, NULL);
		diff = 1000000 * (end1.tv_sec - start1.tv_sec) + end1.tv_usec - start1.tv_usec;
		ReadTime4 += diff;
		...
		Debug::notifyError("read error for %s\n", _file);
		return -1;
	}
}
...
int nrfsRename(nrfs fs, const char* _oldpath, const char* _newpath)
{
	...
	Debug::debugItem("from: %s to: %s", _oldpath, _newpath);
	//printf("nrfsRename: from = %s to = %s\n", _oldpath, _newpath);
	int result;
