...
mm(_mm), mmSize(_mmSize), conf(_conf), MaxNodeID(1), Mode(_Mode) {
	...
	if (isServer) {
		...
		MyNodeID = conf->getIDbyIP(ip);
        	Debug::notifyInfo("IP = %s, NodeID = %d HostName=%s", ip.c_str(), MyNodeID, hname);
		//for(int i=0 ;hent->h_addr_list[i]; i++) {
		//	string ip(inet_ntoa(*(struct in_addr*)(hent->h_addr_list[i])));
		//	Debug::notifyInfo("List: IP[%d] %s", i, ip.c_str());
		//}
	} else {
        cqPtr = 0;
    }
...
bool RdmaSocket::_RdmaBatchSend(uint16_t NodeID, uint64_t SourceBuffer, uint64_t BufferSize, int BatchSize) {
	...
	signal = signal || ((peer->counter & SIGNAL_BATCH) == 0);
    ...
    if(signal && peer->outstanding && !isServer)
	    PollCompletion(NodeID, 1, &wc);

    peer->outstanding = peer->outstanding + signal;

    Debug::debugItem("Post RDMA_SEND: batchsize = %d signaled:%s", BatchSize, signal?"yes":"no");
...
bool RdmaSocket::RdmaOrderedRead(uint16_t NodeID, uint64_t SourceBuffer, uint64_t DesBuffer, uint64_t BufferSize, int TaskID, uint32_t *assigned_wr_id) {
    //assert(peers[NodeID]);
    struct ibv_sge sg;
    struct ibv_send_wr wr;
    struct ibv_send_wr *wrBad;
    ...
    *assigned_wr_id = (uint32_t)wr.wr_id;
    Debug::debugItem("Post RDMA_READ wr_id %lu remote address = %lx", wr.wr_id, wr.wr.rdma.remote_addr);
...
bool RdmaSocket::_RdmaBatchRead(uint16_t NodeID, uint64_t SourceBuffer, uint64_t DesBuffer, uint64_t BufferSize, int BatchSize) {
    ...
    Debug::debugItem("Post RDMA_READ: batchsize = %d signaled:%s", BatchSize, signal?"yes":"no");
...
bool RdmaSocket::InboundHamal(int TaskID, uint64_t bufferSend, uint16_t NodeID, uint64_t bufferReceive, uint64_t size) {
    ...
    while (TotalSizeSend < size) {
        ...
        //                1);
        gettimeofday(&start, NULL);
        RdmaOrderedRead(NodeID, SendPoolAddr, bufferReceive + TotalSizeSend, SendSize, TaskID + 1, &wr_id);
        PollOrderedCompletion(NodeID, wr_id, &wc);
...
bool RdmaSocket::RdmaOrderedWrite(uint16_t NodeID, uint64_t SourceBuffer, uint64_t DesBuffer, uint64_t BufferSize, uint32_t imm, int TaskID, uint32_t *assigned_wr_id) {
    //assert(peers[NodeID]);
    struct ibv_sge sg;
    struct ibv_send_wr wr;
    struct ibv_send_wr *wrBad;
    ...
    *assigned_wr_id = (uint32_t)wr.wr_id;
    ...
    Debug::debugItem("Post RDMA_WRITE wr_id %lu remote address = %lx", wr.wr_id, wr.wr.rdma.remote_addr);
...
bool RdmaSocket::OutboundHamal(int TaskID, uint64_t bufferSend, uint16_t NodeID, uint64_t bufferReceive, uint64_t size) {
    ...
    while (TotalSizeSend < size) {
        ...
        //                (uint32_t)-1,
        //                1);
        RdmaOrderedWrite(NodeID, SendPoolAddr, bufferReceive + TotalSizeSend, SendSize, -1, TaskID + 1, &wr_id);
        PollOrderedCompletion(NodeID, wr_id, &wc);
...
bool RdmaSocket::_RdmaBatchWrite(uint16_t NodeID, uint64_t SourceBuffer, uint64_t DesBuffer, uint64_t BufferSize, uint32_t imm, int BatchSize) {
	...
	signal = signal || ((peer->counter & SIGNAL_BATCH) == 0);
        ...
        //send_wr[w_i].send_flags |= IBV_SEND_INLINE;
        send_wr[w_i].wr.rdma.remote_addr = DesBuffer + peer->RegisteredMemory + w_i * 4096;
        Debug::debugItem("remote address = %lx, Counter = %d, Outstanding = %d imm = %lx",
			send_wr[w_i].wr.rdma.remote_addr, peer->counter, peer->outstanding, imm);
        send_wr[w_i].wr.rdma.rkey        = peer->rkey;
        peer->counter += 1;
    }
   ...
   if(signal && peer->outstanding && !isServer)
    ...
    peer->outstanding += signal;

    Debug::debugItem("Post RDMA_WRITE: batchsize = %d signaled:%s", BatchSize, signal?"yes":"no");
    if (ibv_post_send(peer->qp[0], &send_wr[0], &wrBad)) {
        Debug::notifyError("Send with RDMA_WRITE(WITH_IMM) failed.");
        printf("%s\n", strerror(errno));
        return false;
...
int RdmaSocket::PollOrderedCompletion(uint16_t NodeID, uint32_t wr_id, struct ibv_wc *wc) {
    int ret=0;
    Debug::debugItem("poll till wr_id %u", wr_id);

    // FIXME: this is just a hackish fix and can still block indefinitely for unordered operations
    // either use separate completion queues for different qps or simply encode both nodeid and
    // seqn in wr_id
    uint32_t last_wr_id = 0;
    while (wr_id > last_wr_id) {
        ret = ibv_poll_cq(peers[NodeID]->cq, 1, wc);
	if(ret > 0) {	
		/* Check Completion Status */
		if (wc->status != IBV_WC_SUCCESS) {
			Debug::notifyError("Failed status %s (%d) for wr_id %lu with opcode %d", 
					ibv_wc_status_str(wc->status),
					wc->status, wc->wr_id, wc->opcode);
        		return -1;
    		}
		else {
			// wr_id between 0 & MaxNodeID means an unordered request completion
			// note: if this is true, wr_id corresponds to the NodeID for the wc
			if(wc->wr_id <= MaxNodeID)
				peers[wc->wr_id]->outstanding = 0;
			else
				last_wr_id = wc->wr_id;
			Debug::debugItem("completion wr_id found %lu required %u",
					wc->wr_id, wr_id);
		}
	}
	else if(ret < 0) {
    ...
    Debug::debugItem("Found Completion Message");
    return 0;
}
...
int RdmaSocket::PollCompletion(uint16_t NodeID, int PollNumber, struct ibv_wc *wc) {
    ...
    Debug::debugItem("poll till %d completions", PollNumber);
    do {
    ...
    Debug::debugItem("Finished Completion Message(s)");
    return count;
}
