...
bool RPCClient::RemoteRead(uint64_t bufferSend, uint16_t NodeID, uint64_t bufferReceive, uint64_t size) {
	std::lock_guard<std::mutex> guard(g_send_lock);
	return socket->RemoteRead(bufferSend, NodeID, bufferReceive, size);
}

bool RPCClient::RemoteWrite(uint64_t bufferSend, uint16_t NodeID, uint64_t bufferReceive, uint64_t size) {
	std::lock_guard<std::mutex> guard(g_send_lock);
	return socket->RemoteWrite(bufferSend, NodeID, bufferReceive, size);
}
...
bool RPCClient::RdmaCall(uint16_t DesNodeID, char *bufferSend, uint64_t lengthSend, char *bufferReceive, uint64_t lengthReceive) {
	...
	if (send->message == MESSAGE_DISCONNECT
		...
		//printf("discard rpc for message type: %d\n", send->message);
