...
bool FileSystem::mknodWithMeta(const char *path, FileMeta *metaFile)
{
    ...
    if ((path == NULL) || (metaFile == NULL)) {
        ...
        AddressHash hashAddress = HashTable::getAddressHash(&hashUnique); /* Get address hash by unique hash. */
        if (checkLocal(hashNode) == true) { /* If local node. */
            Debug::debugItem("Stage 1.1. Check parent.");
            bool result;
	    uint64_t indexMeta;
	    bool isDirectory;

	    // ------ START HOTFIX ------
	    if(storage->hashtable->get(&hashUnique, &indexMeta, &isDirectory) == true) {
	    	Debug::debugItem("Stage 1.2. Path exists; overwrite to adhere to POSIX");
	     	FileMeta oldMetaFile;

		if (storage->tableFileMeta->get(indexMeta, &oldMetaFile) == false) {
			Debug::notifyError("Fail due to get file meta error.");
			return false; /* Fail due to get file meta error. */
		}
		else { 
			if(remove(path, &oldMetaFile) == false) {
				Debug::notifyError("Fail due to file remove error.");
				return false; /* Fail due to file remove error. */
			}
			/*
			else {
				for (uint64_t i = 0; i < oldMetaFile.count; i++) {
					if (checkLocal(oldMetaFile.tuple[i].hashNode) && oldMetaFile.tuple[i].hashNode != 0) 			   					   {
						//TODO: free remote blocks
					}
				}
			}
			*/
	    ...
	    // ------ END HOTFIX -----
            uint64_t key = lockWriteHashItem(hashNode, hashAddress); /* Lock hash item. */
            {
                //if (storage->hashtable->get(&hashUnique, &indexMeta, &isDirectory) == true) { /* If path exists. */
                    //result = false; /* Fail to avoid file overwrite. */
		//}
		//else {
                    Debug::debugItem("Stage 2. Create file meta from old.");
                       ...
                       uint64_t countNode, NodeHash hashLocalNode)
{
    if ((buffer == NULL) || (bufferBlock == NULL) || (countFile == 0) || (countDirectory == 0) ||
        ...
        (hashLocalNode > countNode)) {
     	Debug::notifyError("possible faulty params: buffer %p bufferBlock %p countNode %lu"
			"hashLocalNode %lu countFile %lu countDirectory %lu countBlock %lu",
			buffer, bufferBlock, countNode, hashLocalNode, countFile, countDirectory,
			countBlock);
