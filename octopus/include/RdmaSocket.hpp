...
public:
	...
	int PollOrderedCompletion(uint16_t NodeID, uint32_t wr_id, struct ibv_wc *wc);
	...
	*return true on success, false on error.
	**/
	bool RdmaOrderedRead(uint16_t NodeID, uint64_t SourceBuffer, uint64_t DesBuffer, uint64_t BufferSize, int TaskID, uint32_t *wr_id);
	...
	*return true on success, false on error.
	**/
	bool RdmaOrderedWrite(uint16_t NodeID, uint64_t SourceBuffer, uint64_t DesBuffer, uint64_t BufferSize, uint32_t imm, int TaskID, uint32_t *wr_id);
