...
inline void time_stats_stop(struct time_stats* ts)
{
   ...
   double start_sec = (double)(ts->time_start.tv_sec * NS_IN_SEC + (double)ts->time_start.tv_nsec) / NS_IN_SEC;
   double end_sec = (double)(end.tv_sec * NS_IN_SEC + (double)end.tv_nsec) / NS_IN_SEC;

   double sec = end_sec - start_sec;
#endif

   //double start_nsec = (double)(ts->time_start.tv_sec * NS_IN_SEC + (double)ts->time_start.tv_nsec);
   //double end_nsec =   (double)(end.tv_sec * NS_IN_SEC + (double)end.tv_nsec);
   //printf("\ttime: %.3f msec %lf ns\n", sec, end_nsec - start_nsec);
