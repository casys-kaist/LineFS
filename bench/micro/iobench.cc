	...
	test_type(_test_type), zipf_file(_zipf_file)
{
	...
	test_file += "/" + std::string(test_file_name) + std::to_string(0) + "-" + std::to_string(dev_id) + "-" + std::to_string(id);
	per_thread_stats = 0;
}
...
void io_bench::do_write(void)
{
	...
	if (test_type == SEQ_WRITE) {
		...
		while (1){
			...
			if (cnt == cnt_max)
				break;
			cout << "iobench Run cnt=" << cnt << " ops=" << ops_cap << endl;
			cnt++;
#else
			if (per_thread_stats) {
				time_stats_init(&stats, 1);
...
{
	std::cerr << "usage: " << prog
		<< " [-d <directory>] [-f <file-prefix>] [-n <# of ops>] [-s 'fsync'] <wr/sr/sw/rr/rw/zr/zw/zm>"
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	if (wait_signal) {
	    ...
	    *shm_proc += 1;
	    printf("shm_proc value added. Now shm_proc=%d\n", *shm_proc);
	    fflush(stdout);
	    while (*shm_proc > 0){
		usleep(1000000);
		printf("shm_proc=%d\n", *shm_proc);
	    }
	    printf("Do shutdown_fs\n");
	    shutdown_fs();
	} else {
	    shutdown_fs();
	}
	printf("Shutdown FS done.\n");
	fflush(stdout);
#endif
