...
static void bm_setup(void)
{
    ...
    bm_state.fds[1] = open(dev_paths[0], O_RDWR | O_CREAT | O_TRUNC, 0666);
    bm_state.fds[2] = open(dev_paths[0], O_RDWR | O_CREAT | O_TRUNC, 0666);
...
ssize_t bm_read(int fd, void *buf, size_t count)
{
    ...
    for ( ; buf_offset < count ; buf_offset += block_size) {
        ...
        dbg("Not cached, gotta go to disk, block %lld\n", elt->block_offset_on_file);
        
        uint64_t file_seek = elt->block_offset_on_file * block_size;
        lseek(bm_state.fds[2], file_seek, SEEK_SET);
