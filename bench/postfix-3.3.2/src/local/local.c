...
static void local_before_exit(char *unused_service_name, char **unused_argv)
{
#ifdef MLFS
    msg_info("[ASSISE] Do shutdown_fs()");
    shutdown_fs();
    msg_info("[ASSISE] shutdown_fs() done.");
#endif
}
...
int     main(int argc, char **argv, char **envp)
{
    ...
    // Print envp.
    msg_info("[ASSISE] Local main envp");
    for (i = 0; envp[i] != NULL; i++)
    {    
	msg_info("[ASSISE] %d: %s", i, envp[i]);
    }
    ...
    single_server_main(argc, argv, local_service,
		       ...
		       CA_MAIL_SERVER_BOUNCE_INIT(VAR_LOCAL_DSN_FILTER,
                       ...
                       CA_MAIL_SERVER_EXIT(local_before_exit),
		       0);
}
