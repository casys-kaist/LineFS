...
int     deliver_maildir(LOCAL_STATE state, USER_ATTR usr_attr, char *path)
{
    ...
    //msg_info("[ASSISE] path: %s\n", path);
    // char *base = sane_basename(buf, path);
    char *dir  = sane_dirname(buf, path);
    char *username = sane_basename(buf, dir);
    //msg_info("[ASSISE] base: %s, dir: %s, username: %s\n", base, dir, username);

    // Trim usernames to make their size to be <= 24.
    if (strcmp(username, "david_juliana_at_datascope") == 0) {
	username = "david_juliana_at_datasco";
    } else if (strcmp(username, "enron.com.dcastro.jfrizzel") == 0) {
        username = "enron.com.dcastro.jfrizz";
    } else if (strcmp(username, "forbes_t.jefferson_po.hw_email") == 0) {
        username = "forbes_t.jefferson_po.hw";
    } else if (strcmp(username, "mark_-_ect_legal_taylor.ect") == 0) {
        username = "mark_-_ect_legal_taylor.";
    } else if (strcmp(username, "solardevelopmentcooperative") == 0) {
        username = "solardevelopmentcooperat";
    } else if (strcmp(username, "taylor_c.jefferson_po.hw_email") == 0) {
        username = "taylor_c.jefferson_po.hw";
    } else if (strcmp(username, "watson_c.jefferson_po.hw_email") == 0) {
        username = "watson_c.jefferson_po.hw";
    } else if (strcmp(username, "hill_l.jefferson_po.hw_email") == 0) {
        username = "hill_l.jefferson_po.hw_e";
    }

    // Assise supports dir size < 28
    assert(strlen(username) <= 24);

    //path = concatenate("/mlfs/", username, "/Maildir/", (char *) 0);
    //msg_info("[ASSISE] %s\n", path);

    newdir = concatenate("/mlfs/", username, "/Maildir/", "new/", (char *) 0);
    tmpdir = concatenate("/mlfs/", username, "/Maildir/", "tmp/", (char *) 0);
    curdir = concatenate("/mlfs/", username, "/Maildir/", "cur/", (char *) 0);

    //newdir = concatenate("/mlfs", "/Maildir/", "new/", (char *) 0);
    //tmpdir = concatenate("/mlfs", "/Maildir/", "tmp/", (char *) 0);
    //curdir = concatenate("/mlfs", "/Maildir/", "cur/", (char *) 0);

    //newdir = concatenate("/mlfs", "/n/", (char *) 0);
    //tmpdir = concatenate("/mlfs", "/t/", (char *) 0);
    //curdir = concatenate("/mlfs", "/c/", (char *) 0);

    // For test. Make dir names short.
    // newdir = concatenate("/mlfs/", "n/", (char *) 0);
    // tmpdir = concatenate("/mlfs/", "t/", (char *) 0);
    // curdir = concatenate("/mlfs/", "c/", (char *) 0);
    ...
    assert(strlen(STR(buf)) < 28); // Check dir size <= 28 (Assise's limitation)

    // For test (short dir name). It might have duplicated names and all mails are saved into the same dir.
    // vstring_sprintf(buf, "%lu", (unsigned long) starttime.tv_usec); // Temp tmp name.
	...
	msg_fatal("fstat %s: %m", tmpfile);
        ...
        // vstring_sprintf(buf, "%lu.V%lxI%lxM%lu.%s",
                        ...
                        // get_hostname());
        // Shorter one. (Assise supports dir size <= 28)
        //vstring_sprintf(buf, "%lu.%lxM%lu.%s",
        //                (unsigned long) starttime.tv_sec,
        //                (unsigned long) st.st_ino,
        //                (unsigned long) starttime.tv_usec,
        //                get_hostname());
      	vstring_sprintf(buf, "%lu.%lxM%lu",
                        (unsigned long) starttime.tv_sec,
                        (unsigned long) st.st_ino,
                        (unsigned long) starttime.tv_usec);
        assert(strlen(STR(buf)) < 28); // Check dir size <= 28 (Assise's limitation)
        ...
        // For test (short dir name). It might have duplicated names and all mails are saved into the same dir.
	//vstring_sprintf(buf, "%lu", (unsigned long) starttime.tv_usec); // Temp new name.
