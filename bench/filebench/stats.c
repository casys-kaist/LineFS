...
//#if 0
	...
	while (flowop) {
		...
		(void) snprintf(line, sizeof(line), "%-20s %dops %8.0lfops/s "
		    "%5.1lfmb/s %8.3f us/op",
		    flowop->fo_name,
		    flowop->fo_stats.fs_count,
		    flowop->fo_stats.fs_count / total_time_sec,
		    (flowop->fo_stats.fs_bytes / MB_FLOAT) / total_time_sec,
		    flowop->fo_stats.fs_count ?
		    (double)flowop->fo_stats.fs_total_lat * 1000 /
		    ((double)flowop->fo_stats.fs_count * SEC2MS_FLOAT) : 0);
		(void) strcat(str, line);

		(void) snprintf(line, sizeof(line)," [%.3f us - %5.3f us]",
			flowop->fo_stats.fs_minlat * 1000 / SEC2MS_FLOAT,
			flowop->fo_stats.fs_maxlat * 1000 / SEC2MS_FLOAT);
		(void) strcat(str, line);
