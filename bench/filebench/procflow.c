...
procflow_shutdown(void)
{
...
#else
                        // Send SIGINT to worker for shutdown_fs() call.
                        // FIXME. SIGUSR1 is not working correctly.
                        (void) kill(pid, SIGINT);
                        sleep (3);

                        // Send signal to workers. (original code)
                        // FIXME De we need to send SIGUSR1?
	...
	(void) ipc_mutex_unlock(&filebench_shm->shm_procs_running_lock);
        ...
        // Shutdown_fs is called by SIGINT.
