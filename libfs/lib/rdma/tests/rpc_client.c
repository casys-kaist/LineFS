...
int main(int argc, char **argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s <peer-address> <iters> [-s sync]\n", argv[0]);
		return 1;
	}
	...
	*/
	...
	init_rdma_agent(portno, regions, 1, 256, add_peer_socket, remove_peer_socket, signal_callback);
	...
	for(int i=0; i<iters; i++) {
		...
		if(msg_sync)
			//spin_till_receive(sockfd);
 			IBV_AWAIT_RESPONSE(sockfd, seqn);
