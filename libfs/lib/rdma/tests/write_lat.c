...
int main(int argc, char **argv)
{
	...
	if (argc != 6 && argc != 1) {
		fprintf(stderr, "usage: %s <peer-address> <src: dram|nvm> <dst: dram|nvm> <io-size> <iters> [-e <sge count>] [-b <batch size>] [-p persist] (note: run without args to use as server)\n", argv[0]);
		return 1;
	}
	...
	*/
	...
	init_rdma_agent(portno, regions, isClient?1:MR_COUNT, 256, add_peer_socket, remove_peer_socket, isClient?test_callback:signal_callback);
	...
	for(int i=0; i<iters; i++) {
		...
		if(data_persist) {
			...
			//printf("setting imm to %u\n", meta[n]->imm);	
		}

		//reached end of batch or iterations
		if(n == (batch_size - 1) || i == iters-1) {
			//printf("ib_post_send\n");
