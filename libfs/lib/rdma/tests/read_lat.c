...
int msg_sync = 1;	//default - synchronous writes
...
int persist_data(uint16_t region_id, uint64_t offset)
{
	addr_t addr = mr_local_addr(sockfd, region_id) + offset;

	//printf("persist until: %lx\n", offset);

	if(!persist_addr || persist_addr > addr) //beginning of benchmark
		persist_addr = mr_local_addr(sockfd, region_id);
	...
	*/
	...
	for(int i=0; i<iters; i++) {
		...
		if(!n || i == iters-1) {
			...
			IBV_WRAPPER_READ_SYNC(sockfd, meta[0], src_mr, dst_mr);

			if(data_persist) {
				persist_data(src_mr, transferred_bytes);
			}
