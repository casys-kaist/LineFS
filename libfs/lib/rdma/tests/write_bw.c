...
uint32_t signal_encode(uint16_t region_id, uint64_t addr)
{
	...
	//printf("region_id %u block_no %u addr %lu aligned_addr %u offset %u\n", region_id,
	//		(aligned_addr >> BLOCK_SIZE_SHIFT), addr, aligned_addr, addr - aligned_addr);
...
{
	uint16_t block_no = (uint16_t) (msg->id >> 16);

	//this assumes that region_id only needs 2 bits (assert MR_COUNT <= 4)
	uint8_t srv_mr = (uint8_t) ((msg->id & 0x00003000uL) >> BLOCK_SIZE_SHIFT);

	uint8_t client_mr = (uint8_t) ((msg->id & 0x0000C000uL) >> (BLOCK_SIZE_SHIFT + 2));

	uint16_t offset = (uint16_t) (msg->id & 0x00000FFFuL);

	addr_t addr = mr_local_addr(msg->sockfd, srv_mr) + (block_no << BLOCK_SIZE_SHIFT) + offset;

	//printf("callback (imm %x) - persist until: region_id %u block_no %u offset %u addr %lx\n",	
	//		msg->id, srv_mr, block_no, offset, addr);

	if(!persist_addr || persist_addr > addr) //beginning of benchmark
		persist_addr = mr_local_addr(msg->sockfd, srv_mr);
	pmem_persist((void *)addr, addr - persist_addr);
	...
	meta->addr = (uintptr_t)mr_remote_addr(msg->sockfd, client_mr);
	meta->length = 0;
	meta->sge_count = 0;
	meta->next = NULL;

	meta->imm = msg->id;
	IBV_WRAPPER_WRITE_WITH_IMM_ASYNC(msg->sockfd, meta, srv_mr, client_mr);

	//printf("Sent ACK on sock %d with imm %x\n", msg->sockfd, meta->imm);
}
...
int main(int argc, char **argv)
{
	...
	if (argc != 6 && argc != 1) {
		fprintf(stderr, "usage: %s <peer-address> <src: dram|nvm> <dst: dram|nvm> <io-size> <iters> [-e <sge count>] [-b <batch size>] [-s sync] [-p persist] (note: run without args to use as server)\n", argv[0]);
		return 1;
	}
	...
	*/
	...
	for(int i=0; i<iters; i++) {
		...
		if(data_persist) {
			meta[n]->imm = signal_encode(dst_mr, transferred_bytes); //signal range of addr to persist
		...
		if(!n || i == iters-1) {
			...
			if(msg_sync) {
				if(data_persist) {
					//printf("meta: addr=%lu length=%u\n", meta[0]->addr, meta[0]->length);
					IBV_WRAPPER_WRITE_WITH_IMM_ASYNC(sockfd, meta[0], src_mr, dst_mr);
					//printf("waiting till response received. n=%d, meta[n]->imm=%u\n", n, meta[n]->imm);
					IBV_AWAIT_RESPONSE(sockfd, meta[n]->imm);
				}
				else
					IBV_WRAPPER_WRITE_SYNC(sockfd, meta[0], src_mr, dst_mr);
			}
			else {
				if(data_persist) {
					if(i == iters - 1)
					...
					wr_id = IBV_WRAPPER_WRITE_ASYNC(sockfd, meta[0], src_mr, dst_mr);
			}
		}
	...
	printf("Throughput: %3.3f MB/s\n",(double)(transfer_size)
			/ (1024.0 * 1024.0 * (double) time_stats_get_avg(timer)));
