...
typedef enum {SEQ_WRITE, SEQ_READ, SEQ_WRITE_READ, RAND_WRITE, RAND_READ, UPDATE, VERIFY,NONE} test_t;
...
{
	public:
		io_test(int _id, unsigned long _file_size_bytes, unsigned int _io_size,
...
io_test::io_test(int _id, unsigned long _file_size_bytes, unsigned int _io_size,
		test_t _test_type, test_mode_t _test_mode) 
...
void io_test::Run(void)
{
	...
	if (test_type == SEQ_READ || test_type == RAND_READ)
		this->do_read();
	else if (test_type == SEQ_WRITE || test_type == RAND_WRITE)
		this->do_write();
...
unsigned long io_test::str_to_size(char* str)
{
...
test_t io_test::get_test_type(char *test_type)
{
	/**
...
test_t io_test::set_test_type(test_t mode)
{
	this->test_mode = mode;
}

test_mode_t io_test::get_test_mode(char *test_mode)
{
...
void io_test::hexdump(void *mem, unsigned int len)
{
	unsigned int i, j;
...
void io_test::show_usage(const char *prog)
{
	std::cerr << "usage: " << prog  
		<< " <wr/sr/sw/rr/rw> <fs>" 
...
{
	std::vector<io_test *> io_workers;
	unsigned long file_size_bytes;
	unsigned int io_size = 0;
	pid_t pid;

	if (argc != 5) {
		io_test::show_usage(argv[0]);
		exit(-1);
	}

	file_size_bytes = io_test::str_to_size(argv[3]);
	io_size = io_test::str_to_size(argv[4]);

	std::cout << "Total file size: " << file_size_bytes << "B" << endl
		<< "io size: " << io_size << "B" << endl;

	io_workers.push_back(new io_test(1, 
				file_size_bytes,
				io_size,
				io_test::get_test_type(argv[1]),
				io_test::get_test_mode(argv[2])));
	...
	make_digest_request(100);

	// do partial update after digest.
	io_workers[0]->set_test_type(UPDATE);
	io_workers[0]->Run();

	io_workers[0]->set_test_type(VERIFY);
	io_workers[0]->Run();
